...
<md-table v-model="items" md-sort="item.score.total" md-sort-order="desc" md-card>
      ...
      <md-table-row slot="md-table-row" slot-scope="{ item }">
        ...
        <md-table-cell md-label="Score" md-sort-by="score.total">{{ item.score.total }}</md-table-cell>
        <md-table-cell md-label="CVE" md-sort-by="cve">{{ item.cve }}</md-table-cell>
        <md-table-cell md-label="Domain" md-sort-by="parsed.domain">{{ item.parsed.domain }}</md-table-cell>
        <md-table-cell><router-link :to="{name: 'Edit', params: { id: item._id, feed: 'url' }}" class="md-raised md-primary">info</router-link></md-table-cell>
      </md-table-row>
    </md-table>  
</template>
