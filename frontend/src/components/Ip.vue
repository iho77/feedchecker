    ...
    <md-table v-model="items" md-sort="item.feed[0].score.total" md-sort-order="desc" md-card>
      ...
      <md-table-row slot="md-table-row" slot-scope="{ item }">
        ...
        <md-table-cell v-if="item.feed[0].res != 'no data'" md-label="CVE" md-sort-by="feed[0].cve">{{ item.feed[0].cve }}</md-table-cell>
         <md-table-cell v-else>none</md-table-cell>
        <md-table-cell v-if="item.feed[0].res != 'no data'" md-label="Related" md-sort-by="feed[0].related.domains">{{ item.feed[0].related.domains }}</md-table-cell>
         <md-table-cell v-else>none</md-table-cell>
        <md-table-cell v-if="item.feed[0].res != 'no data'"><router-link :to="{name: 'Edit', params: { id: item.feed[0]._id, feed: 'ip' }}" class="md-raised md-primary">info</router-link></md-table-cell>
         <md-table-cell v-else>none</md-table-cell>
      </md-table-row>
     </md-table>
</template>
